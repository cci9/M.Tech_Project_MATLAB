...
% Lets Angle find alpha and fi
alpha1=atan((p1y-a1y)/(p1x-a1x));
if (p1y-a1y)>0 && (p1x-a1x)<0
    alpha1=pi+alpha1;
end
if (p1y-a1y)<0 && (p1x-a1x)<0
    alpha1=pi+alpha1;
...
plot_angle1=theta1*180/pi;
% disp([plot_angle1,r1]);

[theta2,r2]=cart2pol(real(z2),imag(z2));
plot_angle2=theta2*180/pi;
% disp([plot_angle2,r1]);

[theta3,r3]=cart2pol(real(z3),imag(z3));
plot_angle3=theta3*180/pi;
% disp([plot_angle3,r1]);

[theta4,r4]=cart2pol(real(z4),imag(z4));
plot_angle4=theta4*180/pi;
% disp([plot_angle4,r1]);

[theta5,r5]=cart2pol(real(z5),imag(z5));
plot_angle5=theta5*180/pi;
% disp([plot_angle5,r1]);

[theta6,r6]=cart2pol(real(z6),imag(z6));
plot_angle6=theta6*180/pi;
% disp([plot_angle6,r1]);

% Lets dispay analytical result on the commant window
fprintf('\t\t\t\t\t\t\t\t****ANALYTICAL RESULTS****\n');
fprintf('A0A1=%f\tA1P1=%f\tP1B1=%f\tA1B1=%f\tB1B0=%f\tA0B0=%f\n',r1,r2,r4,r5,r3,r6);
fprintf('A0A1 angle=%f\tA1P1 angle=%f\tP1B1 angle=%f\nA1B1 angle=%f\t\tB1B0 angle=%f\tA0B0 angle=%f\n',plot_angle1,plot_angle2,plot_angle4,plot_angle5,plot_angle3,plot_angle6);
